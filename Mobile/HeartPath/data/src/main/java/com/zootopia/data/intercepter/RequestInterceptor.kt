...
class RequestInterceptor @Inject constructor(
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        if(response.code == AUTH_TOKEN_EXPIRE_ERROR) {
            ...
            if(checkIfRefreshable(response)) {
                Log.d(TAG, "intercept: refresh 성공, new access token으로 재호출")
                val newRequest = chain.request().newBuilder().addHeader(AUTHORIZATION, BEARER + newAccessToken).build()
    ...
    private fun checkIfRefreshable(response: Response): Boolean {
            ...
            AUTH_TOKEN_EXPIRE_ERROR -> { // 여러 에러들 종합 (에러 메시지로 확인하자.)
                ...
                when (errorResponse.httpStatus) {
                        ...
                        runBlocking {
                            ...
                            getRefreshToken().let { refreshTokenValue ->
                                ...
                                val result = Retrofit.Builder()
                                    ...
                                    .getReAccessToken(refreshTokenValue)
                                ...
                                if (result.isSuccessful) {
                                    ...
                                    Log.d(
                                        TAG,
                                        "intercept: 재발급한 access token ${result.body()!!.data}"
                                    )
                                    newAccessToken = result.body()!!.data
                                    isRefreshable = true
                                }
                                if (result.body() == null) {
                                    Log.d(
                                        TAG,
                                        "intercept: refresh token로 재발급 실패"
                                    )
                    ...
                    "Auth-004" -> { // 엑세스 토큰 invalid 신호
                        Log.d(
                            TAG,
                            "intercept: 에러(Auth-004) : 해당 토큰은 엑세스 토큰이 아닙니다."
                        )
                    }
